# Veyro Hardware Bill of Materials Guide
...
## Engineering decision: what fits and what does not
The listed ESP32-WROOM-32 DevKit, sensor breakouts, display breakout, charger board, battery, motor, and buttons can fit together electrically on a supervised breadboard or prototype PCB, but the repository does not prove that they fit inside a wrist enclosure. The DevKit headers, USB connector, charger board, battery thickness, and display breakout are the likely mechanical blockers. Treat this combination as a bench prototype until a cardboard or CAD mockup passes clearance, service, skin-contact, and strap tests.

The listed TP4056 board is not an approved power architecture by name alone. TP4056 identifies a charging controller, not guaranteed battery protection, load sharing, thermal behavior, or safe operation while the load is attached. Do not connect a LiPo to a random TP4056 module and then connect its output directly to the ESP32 3V3 pin. The approved prototype decision is: use a known protected 1S LiPo; use a charger board with documented protection and power-path/load-sharing behavior; use a regulated output compatible with the ESP32 input; and verify every rail with a multimeter and current-limited supply before connecting the battery.

Recommended replacement path when the present power stack fails: replace the unknown TP4056 module with a documented 1S LiPo charger/power-path board that provides a regulated 5 V output for the DevKit VIN/5V input, or replace the whole power section with a documented protected charger plus a 3.3 V buck-boost regulator sized for ESP32 transmit peaks and motor startup. The exact part number remains MUST CONFIRM FROM DATASHEET because the battery, enclosure, charging current, connector, and regulator thermal limits are not specified here. A 5 V boost board may make the bench DevKit practical but is usually too large and inefficient for a final watch. For the wearable revision, replace the DevKit with a custom PCB or compact ESP32 module carrier and place the charger, protection, regulator, and test points on that board.

Power acceptance is blocked until these values are recorded: battery maximum and minimum voltage, charger current, regulator input range, regulator output tolerance, ESP32 peak current, motor stall current, sensor current, display current, total charge temperature, total run current, and enclosure temperature. Do not substitute a part merely because its connector matches. Match voltage, current, protection, dimensions, mounting, firmware pins, and supplier documentation.

## Stage checklists

## Stage 1: part identity
